...
class WatchResultPage extends StatefulWidget {
  ...
  @override
  State<WatchResultPage> createState() => _WatchResultPageState();
}
...
class _WatchResultPageState extends State<WatchResultPage> {
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          _judgePigmentationResult(),
          _judgeHydrationResult(),
          _judgeElasticityResult(),
          _judgeMoistureRetentionResult(),
          _judgeSensiticityResult(),
          Padding(
            ...
            child: Container(
              ...
              padding: const EdgeInsets.all(10), // 박스의 안쪽 여백 설정
  ...
  Widget _spiderChart() {
    ...
    if (baumannScores == null) {
      return const Text("결과를 보여줄 수 없습니다.");
    }
  ...
  Widget _judgePigmentationResult() {
    if (widget.resultData?.baumannScores['pigmentation'] / 57 * 100 >= 75) {
      return const Text('색소침착도 : 좋음');
    } else {
      return const Text('색소침착도 : 보통');
    }
  }

  Widget _judgeHydrationResult() {
    if (widget.resultData?.baumannScores['hydration'] / 44 * 100 >= 75) {
      return const Text('유수분 밸런스 : 좋음');
    } else {
      return const Text('유수분 밸런스 : 보통');
    }
  }

  Widget _judgeElasticityResult() {
    if (widget.resultData?.baumannScores['elasticity'] / 85 * 100 >= 75) {
      return const Text('탄력 : 좋음');
    } else {
      return const Text('탄력 : 보통');
    }
  }

  Widget _judgeMoistureRetentionResult() {
    if (widget.resultData?.baumannScores['moistureRetention'] >= 65) {
      return const Text('수분 보유력 : 좋음');
    } else {
      return const Text('수분 보유력 : 보통');
    }
  }

  Widget _judgeSensiticityResult() {
    if (widget.resultData?.baumannScores['sensitivity'] / 64 * 100 >= 75) {
      return const Text('민감도 : 좋음');
    } else {
      return const Text('민감도 : 보통');
    }
  }
